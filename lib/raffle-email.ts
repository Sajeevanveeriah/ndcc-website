...
export async function sendPaidRaffleEmails(orderId: string, eventId: string) {
  ...
  if (!order.customer_email_sent_at) {
    ...
    const receiptData = {
      ...
      paymentDate: String(order.paid_at || order.updated_at || new Date().toISOString()),
      amountCents: Number(order.amount_cents),
      paymentType: 'Raffle Ticket Purchase',
      paymentMethod: 'Stripe Checkout',
      reference: references.length === 1 ? references[0] : `${references[0]} + ${references.length - 1} more`,
    ...
    if (result.status !== 'sent' && result.status !== 'simulated') throw new Error(result.reason);
