...
export async function processBalanceReminders(request: Request) {
  ...
  while(Date.now()<deadline && result.sent+result.failed+result.cancelled<30) {
    ...
    if(order.deleted_at||order.order_status==='cancelled'||order.payment_status!=='part_paid'||cents<=0||(job.delivery&&job.balance_cents!==cents)) {
    ...
    const delivery:EmailPayload=job.delivery||{
      ...
      html:emailHtml('Your apparel order balance',`<p>Hi ${escapeEmailHtml(order.customer_name)},</p><p>Thank you for your part payment towards apparel order <strong>${escapeEmailHtml(order.payment_reference)}</strong>.</p><table role="presentation"><tr><td>Order total</td><td>AUD ${Number(order.total_amount).toFixed(2)}</td></tr><tr><td>Paid so far</td><td>AUD ${Number(order.amount_paid).toFixed(2)}</td></tr><tr><td><strong>Balance due</strong></td><td><strong>AUD ${(cents/100).toFixed(2)}</strong></td></tr></table><p><strong>Full payment is required to process your order.</strong></p><p><a href="${escapeEmailHtml(paymentUrl)}">Pay your remaining balance securely with Stripe</a></p><p>The payment will be added to the same order reference. You can also choose Pay apparel balance under Shop on the club website and enter your order reference and email address.</p><p>We will remind you every three weeks from your order date while a part-paid balance remains. If you have just paid by bank transfer, please allow the club time to confirm it. Reply to this email if you need help.</p><p>Thank you,<br>Newcomb &amp; District Cricket Club</p>`),
    };
