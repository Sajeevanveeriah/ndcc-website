...
export async function sendRegistrationEmail(supabase: ServerClient, entryId: string) {
  ...
  const delivery = job.delivery || {
    ...
    html: emailHtml('Dino Coach registration received',
      ...
      <p>Your starting budget is 15,000,000 virtual Dino Dollars. Fill your first squad with 15 players within five days${deadline ? `, by ${escapeEmailHtml(deadline)}` : ''}. We will remind you while it is incomplete. After the deadline, ask Saj and Rick to reactivate your team. This requirement ends permanently when you complete your first squad.</p>
      <p>Your Dino Coach user manual is attached. It covers registration, squad selection, scoring, the live wallet, player sales, team trades, competition rules and help. You can also <a href="${DINO_MANUAL_URL}">download the user manual (PDF)</a>.</p>
