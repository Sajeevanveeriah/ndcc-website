...
// Keep each independent gate visible. Reactivation only resets the deadline;
// it must never imply that a participant accepted new rules or paid an entry.
export function managerEligibilityIssues(manager: EligibilityManager, entry: EligibilityEntry, rulesVersion: string, now = new Date()): EligibilityIssue[] {
  ...
  if (!manager.first_squad_completed_at && Date.parse(manager.initial_squad_due_at) <= now.getTime()) issues.push({ code: 'expired', message: 'Your initial five-day squad deadline has passed. Open My account to request reactivation.' });
