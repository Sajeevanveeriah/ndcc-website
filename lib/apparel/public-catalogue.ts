...
async function attachOptions(supabase: ReturnType<typeof createServerClient>, products: ProductRow[]) {
  ...
  if (error) {
    if (error.code === '42P01' || error.code === 'PGRST205') return products.map((p) => ({ ...p, options: [] }));
    throw new Error('Catalogue options unavailable');
  }
...
export async function loadPublicCatalogue() {
  if (!isServerSupabaseConfigured()) throw new Error('Catalogue unavailable');
  ...
  if (initial.error && isMissingKnownColumn(initial.error.message)) {
    ...
    if (fallback.error) throw new Error('Catalogue unavailable');
    const withOptions = await attachOptions(supabase, (fallback.data ?? []) as ProductRow[]);
    return withOptions;
  }

  if (initial.error) throw new Error('Catalogue unavailable');
