...
import { buildStaffOrderNotificationContent } from '@/lib/order-notification-content';
...
export async function sendOrderPaymentReceiptForPayment(
  ...
  const result = await sendEmail({
    ...(await getReceiptRecipients(order.customer_email, department ? await getStaffOrderNotificationRecipients(department) : [])),
