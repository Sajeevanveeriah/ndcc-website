...
export default function TransfersClient() {
  const [data, setData] = useState<any>(null); const [outId, setOutId] = useState(''); const [inId, setInId] = useState(''); const [feedback, setFeedback] = useState<string | null>(null); const [error, setError] = useState<string | null>(null);
  const load = () => fantasyJsonFetch<any>('/api/fantasy/transfers').then(setData).catch((err) => setError(err.message));
  useEffect(() => { load(); }, []);
