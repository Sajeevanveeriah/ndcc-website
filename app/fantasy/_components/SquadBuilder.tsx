...
type Player = { id: string; display_name: string; role: string; team_label: string | null; price_dino_dollars: number; source_status: string; published_at: string | null };
...
export default function SquadBuilder({ readonlyMode = false }: { readonlyMode?: boolean }) {
  ...
  const used = selection.reduce((sum, item) => sum + (readonlyMode ? item.purchasePriceDinoDollars : players.find((player) => player.id === item.playerId)?.price_dino_dollars ?? item.purchasePriceDinoDollars), 0);
  ...
  const save = async (mode: 'draft' | 'submit') => { setSaving(true); setError(''); try {
    const result = await fantasyJsonFetch<{ selection: Pick[] }>(`/api/fantasy/squad${query}`, { method: 'POST', body: JSON.stringify({ selection, mode }) });
