...
export default function LeaguesClient() {
 const [data,setData]=useState<any>({leagues:[]}); const [name,setName]=useState(''); const [code,setCode]=useState(''); const [error,setError]=useState<string|null>(null); const [feedback,setFeedback]=useState<string|null>(null);
 const load=()=>fantasyJsonFetch<any>('/api/fantasy/leagues').then(setData).catch((err)=>setError(err.message)); useEffect(()=>{load();},[]);
