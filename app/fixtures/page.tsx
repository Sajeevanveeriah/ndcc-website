...
export default async function FixturesPage() {
  const [blocks, teamLinks, settings] = await Promise.all([
    ...
    <>
      <section className="page-hero">
        <div className="container-width">
          <h1 className="page-hero-title">{blocks['fixtures.hero']?.title || ''}</h1>
          <p className="page-hero-subtitle">
            {blocks['fixtures.hero']?.body || ''}
          </p>
        </div>
      </section>
        ...
        <div className="container-width">
          <Card className="border-l-4 border-l-maroon-700">
            <CardContent className="p-8">
              ...
              <Link
                href={blocks['fixtures.status']?.cta_url || settings.playhq_org_url || '#'}
                target="_blank"
                rel="noopener noreferrer"
                className="btn-primary inline-flex items-center"
              >
                {blocks['fixtures.status']?.cta_label || ''}
      ...
      <section className="section-padding bg-sky-50">
        <div className="container-width">
          <h2 className="section-title mb-8">{blocks['fixtures.team_links']?.title || ''}</h2>
          <p className="text-gray-600 font-body mb-8 max-w-3xl">
            {blocks['fixtures.team_links']?.body || ''}
          </p>
                  ...
                  <CardContent className="p-5">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-maroon-700 font-body text-sm font-semibold group-hover:underline inline-flex items-center">
                        {blocks['fixtures.team_links']?.cta_label || ''}
