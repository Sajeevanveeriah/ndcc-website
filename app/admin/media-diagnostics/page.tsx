    ...
    <div className="space-y-6">
      ...
      <section className="bg-white border rounded-xl p-5 space-y-3">
        ...
        <dl className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
          <div><dt className="text-gray-500">GITHUB_CONTENTS_TOKEN</dt><dd className="font-semibold">{status?.githubTokenPresent ? 'Present' : 'Missing'}</dd></div>
          <div><dt className="text-gray-500">GITHUB_REPO_OWNER</dt><dd className="font-semibold">{status?.repoOwnerPresent ? status.repoOwnerPreview ?? 'Present' : 'Missing'}</dd></div>
          <div><dt className="text-gray-500">GITHUB_REPO_NAME</dt><dd className="font-semibold">{status?.repoNamePresent ? status.repoNamePreview ?? 'Present' : 'Missing'}</dd></div>
          <div><dt className="text-gray-500">GITHUB_CONTENTS_BRANCH</dt><dd className="font-semibold">{status?.branchPresent ? status.branchPreview ?? 'Present' : 'Missing'}</dd></div>
          <div><dt className="text-gray-500">GITHUB_MEDIA_BASE_PATH</dt><dd className="font-semibold">{status?.basePathPresent ? status.basePathPreview ?? 'Present' : 'Missing'}</dd></div>
