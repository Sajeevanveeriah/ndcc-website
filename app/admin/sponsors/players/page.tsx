  ...
  return <div>
    ...
    <Modal isOpen={open} onClose={() => { if (!busy && !uploading) setOpen(false); }} title={id ? 'Edit player sponsor' : 'Add player sponsor'} size="lg">
      <form className="space-y-4" onSubmit={(event) => { event.preventDefault(); void save(); }}>
        ...
        <Input id="player-sponsor-website" label="Sponsor website (optional)" placeholder="example.com, https://example.com or http://example.com" value={form.website} onChange={(event) => setForm({ ...form, website: event.target.value })} />
        <Input id="player-sponsor-order" label="Sponsor order within player (lower appears first)" type="number" min={-100000} max={100000} step={1} value={form.sort_order} onChange={(event) => setForm({ ...form, sort_order: Number(event.target.value) })} />
