...
export default function FantasyReconciliationPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-display font-bold text-gray-900">Historical Fantasy reconciliation</h1>
        <p className="mt-1 max-w-4xl text-sm text-gray-600 font-body">Review Legacy / Unverified statistics against official PlayHQ source data. This workflow is read-only against match statistics: it prepares evidence, export files and SQL proposals only.</p>
      </div>

      {message && <div className="rounded-lg border border-blue-200 bg-blue-50 p-4 text-sm text-blue-900" role="status">{message}</div>}

      <Card><CardContent>
        <h2 className="text-lg font-display font-bold text-gray-900">Create review run</h2>
        <p className="mt-1 text-sm text-gray-600">Choose the reviewed target Fantasy season. Legacy rows are never reassigned here.</p>
        <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-end">
          <label className="text-sm font-semibold text-gray-800">Target season
            <select className="mt-1 block w-full rounded-lg border border-gray-300 px-3 py-2" value={targetSeasonId} onChange={(e) => setTargetSeasonId(e.target.value)}>
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
        <Card><CardContent>
          <h2 className="text-lg font-display font-bold text-gray-900">Runs</h2>
          <div className="mt-4 space-y-3">
            {runs.map((run) => (
              <button key={run.id} type="button" onClick={() => loadRows(run)} className="block w-full rounded-lg border border-gray-200 p-3 text-left hover:bg-gray-50 focus-ring">
                <span className="block text-sm font-semibold text-gray-900">{new Date(run.created_at).toLocaleString()}</span>
                <span className="mt-1 block text-xs text-gray-600">Rows {run.stats_row_count} · exact {run.exact_count} · review {run.review_count} · rejected {run.rejected_count}</span>
              </button>
            ))}
            {!runs.length && <p className="text-sm text-gray-500">No reconciliation runs yet.</p>}
          </div>
        </CardContent></Card>
        ...
        <Card><CardContent>
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <h2 className="text-lg font-display font-bold text-gray-900">Review rows</h2>
              <p className="mt-1 text-sm text-gray-600">Only deterministic exact matches can be bulk approved. Ambiguous or conflicting rows stay quarantined.</p>
            </div>
            {selectedRun && <div className="flex gap-2"><Button type="button" variant="secondary" onClick={bulkApproveExact} disabled={busy}>Approve exact only</Button><a className="rounded-lg border border-gray-300 px-3 py-2 text-sm font-semibold" href={`/api/admin/fantasy/reconciliation?export=${selectedRun.id}`}>Export CSV</a></div>}
          </div>
          <div className="mt-4 overflow-x-auto">
            <table className="min-w-full text-left text-sm">
              <thead><tr className="border-b text-xs uppercase text-gray-500"><th className="py-2 pr-3">Class</th><th className="py-2 pr-3">Status</th><th className="py-2 pr-3">Fixture</th><th className="py-2 pr-3">Evidence</th><th className="py-2 pr-3">Score delta</th></tr></thead>
              <tbody>{rows.map((row) => <tr key={row.id} className="border-b align-top"><td className="py-2 pr-3 font-semibold">{row.classification.replace(/_/g, ' ')}</td><td className="py-2 pr-3">{row.review_status}</td><td className="py-2 pr-3">{row.match_date || 'No date'}<br />{row.opponent || 'No opponent'}</td><td className="py-2 pr-3">{row.review_reason}<br /><span className="text-xs text-gray-500">{row.playhq_game_id || 'No PlayHQ game'} · {row.source_hash ? 'hash captured' : 'no hash'}</span></td><td className="py-2 pr-3">{row.predicted_fantasy_score_delta}</td></tr>)}</tbody>
            </table>
            {!rows.length && <p className="mt-4 text-sm text-gray-500">Select a run to review rows.</p>}
          </div>
