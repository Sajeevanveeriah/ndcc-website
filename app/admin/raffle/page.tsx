...
export default function AdminRafflePage() {
  ...
  return <div className="space-y-6"><div><h1 className="text-2xl font-display font-bold">Raffle</h1><p className="text-content-muted">{campaign ? `${campaign.name}: AUD ${(campaign.price_cents / 100).toFixed(2)} per ticket. ${campaign.draw_label || ''}` : 'Choose a raffle campaign.'}</p>{currentlyVisible&&<Link className="text-maroon-700 underline" href={campaign?.code === 'NDCCRRO' ? '/reverse-raffle' : '/raffle'} target="_blank">Open public raffle page</Link>}</div>
    ...
    <div className="rounded-lg border border-edge-subtle bg-surface-card p-4"><p className="font-bold">Ticket issuing rule</p><p className="text-sm text-content-muted">References use {campaign?.code === 'NDCCRRO' ? '201-300 (NDCCRRO-20260201 to NDCCRRO-20260300)' : 'NDCCRAF-26XXXX'}. Tickets and emails are created only after Stripe confirms payment. Staff notifications go to the club, vice-president and secretary raffle recipients.</p></div>
