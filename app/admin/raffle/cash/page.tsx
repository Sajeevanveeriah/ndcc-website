...
export default function CashRaffle(){
 ...
 setKey(saleKey);
 const res=await fetch(`/api/admin/raffle/cash?sale=${encodeURIComponent(saleKey)}`,{cache:'no-store'});const data=await res.json();if(!res.ok)throw new Error(data.error);setCampaign(data.campaign);if(data.sale)setSale(data.sale);}catch(reason){setError(reason instanceof Error?reason.message:'Unable to load raffle.');}},[]);
