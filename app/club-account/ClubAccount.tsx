...
export default function ClubAccount() {
 ...
 <nav aria-label="Club services" className="grid gap-3 sm:grid-cols-2">{[['/news','Club news'],['/calendar','Club calendar'],['/player-registration','Player registration'],['/join','Social membership'],['/pot-club','Pot Club'],['/fantasy/account','Dino Coach']].map(([href,label])=><Link className="btn-secondary" key={href} href={href}>{label}</Link>)}</nav>
