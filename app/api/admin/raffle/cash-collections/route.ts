...
export async function PATCH(request:Request){
 ...
 const {data,error}=await createServerClient().from('raffle_orders').update({cash_handed_in_at:new Date().toISOString(),cash_handed_in_by:auth.user.id}).eq('id',body.value.orderId).eq('payment_method','cash').eq('status','paid').is('cash_handed_in_at',null).select('id').maybeSingle();
