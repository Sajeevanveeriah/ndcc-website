...
export async function GET(request: Request) {
  ...
  try {
    ...
    const rows: Array<{ id: string; kind: string; reference: string; name: string; amount_cents: number; selected_at: string }> = [];
    for (const kind of ['order', 'raffle', 'dino']) {
      const table = kind === 'order' ? 'orders' : kind === 'raffle' ? 'raffle_orders' : 'fantasy_entries';
      const fields = kind === 'order' ? 'id,payment_reference,customer_name,balance_due,bank_transfer_selected_at' : kind === 'raffle' ? 'id,payment_reference,customer_name,amount_cents,bank_transfer_selected_at' : 'id,payment_reference,entry_fee_cents,bank_transfer_selected_at,fantasy_managers(display_name)';
    ...
    if (new URL(request.url).searchParams.get('format') === 'csv') return new NextResponse(toCsv([
      ...
      ...rows.map(row => [row.kind,row.reference,row.name,(row.amount_cents/100).toFixed(2),row.selected_at,'Purchaser selection only - receipt not confirmed']),
