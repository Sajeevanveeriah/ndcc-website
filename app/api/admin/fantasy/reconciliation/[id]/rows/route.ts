...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  let query = supabase.from('fantasy_historical_reconciliation_rows').select('*').eq('run_id', id).order('created_at');
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (action === 'bulk_approve_exact') {
    const { error } = await supabase.from('fantasy_historical_reconciliation_rows').update({ review_status: 'approved', reviewed_by: user.email, reviewed_at: new Date().toISOString() }).eq('run_id', id).eq('classification', 'exact_match').eq('review_status', 'pending');
    if (error) return NextResponse.json({ success: false, error: error.message }, { status: 500, headers: noStore });
    await supabase.from('fantasy_historical_reconciliation_audit').insert({ run_id: id, action: 'bulk_exact_approved', actor: user.email, detail: { scope: 'exact_match_only' } });
  ...
  const { data: row, error: rowError } = await supabase.from('fantasy_historical_reconciliation_rows').select('classification').eq('id', rowId).eq('run_id', id).maybeSingle();
  ...
  const { error } = await supabase.from('fantasy_historical_reconciliation_rows').update({ review_status: status, reviewed_by: user.email, reviewed_at: new Date().toISOString() }).eq('id', rowId).eq('run_id', id);
  if (error) return NextResponse.json({ success: false, error: error.message }, { status: 500, headers: noStore });
  await supabase.from('fantasy_historical_reconciliation_audit').insert({ run_id: id, row_id: rowId, action: status === 'approved' ? 'row_approved' : status === 'rejected' ? 'row_rejected' : 'row_deferred', actor: user.email, detail: { classification: row.classification } });
  return NextResponse.json({ success: true }, { headers: noStore });
}
