...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (entry?.data?.id) {
      // The account page polls this endpoint, so only retry the welcome email
      // when its delivery job is due (unsent, past backoff, not leased).
      const entryId = entry.data.id;
      const job = await supabase.from('fantasy_registration_emails').select('sent_at,next_attempt_at,lease_until').eq('entry_id', entryId).maybeSingle();
      if (job.error) console.error('[fantasy-manager] Welcome status unavailable', job.error.message);
      else if (registrationEmailRetryDue(job.data)) {
    ...
    console.error('[fantasy-manager] GET failed', { userId: user.id, message: error instanceof Error ? error.message : String(error) });
    return NextResponse.json({ success: false, error: 'Could not load your fantasy manager profile. Please try again or contact the club.' }, { status: 500 });
  }
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (isNewManager || teamName !== existingManager?.team_name) {
      const moderationLog = await supabase.from('fantasy_team_name_moderation').insert({ manager_id: manager.id, submitted_name: teamName, resulting_name: moderation.status === 'approved' ? teamName : null, status: moderation.status, reason: moderation.matchedTerm ? 'Matched a committee-managed blocked term.' : 'Passed deterministic blocked-term checks.' });
      // The audit row is secondary to the saved profile; record the failure for follow-up.
      if (moderationLog.error) console.error('[fantasy-manager] Team-name moderation log failed', { managerId: manager.id, code: moderationLog.error.code, message: moderationLog.error.message });
    }
    ...
    console.error('[fantasy-manager] POST failed', { userId: user.id, message: error instanceof Error ? error.message : String(error) });
    return NextResponse.json({ success: false, error: 'Could not save your fantasy manager profile. Please try again or contact the club.' }, { status: 500 });
  }
}
