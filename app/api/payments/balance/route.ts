...
export async function POST(request: Request) {
  ...
  let query = db.from('orders').select('id,payment_reference,customer_email,total_amount,amount_paid,balance_due,payment_status,order_status').eq('order_category','merch').is('deleted_at',null);
  ...
  if (!order || (!token && String(order.customer_email).trim().toLowerCase() !== email)) return NextResponse.json({error:'No matching apparel order was found. Check your reference and email.'},{status:404,headers:noStore});
  if (body.checkout === true) {
    ...
    return createCheckout(new Request(new URL('/api/payments/checkout-session',request.url), {
      method:'POST',headers:request.headers,body:JSON.stringify({order_id:order.id,return_path:'/merchandise'}),
    }));
  }
  return NextResponse.json({reference:order.payment_reference,total:Number(order.total_amount),paid:Number(order.amount_paid),balance:Number(order.balance_due),status:order.payment_status,cancelled:order.order_status==='cancelled'}, {headers:noStore});
}
