...
for(const status of ['unpaid','pending','pending_bank_transfer']) {
 await test(`${status} order gets a reminder and can pay using its private link`,async()=>{reset();Object.assign(orders[0],{payment_status:status,amount_paid:0,balance_due:195});const result=await reminders.processBalanceReminders(request({}));assert.equal(result.sent,1);assert.match(emails[0].html,/Payment is still outstanding/);assert.doesNotMatch(emails[0].html,/Thank you for your part payment/);assert.match(emails[0].html,/AUD 195.00/);assert.equal((await balance.POST(request({token:links[0].token,checkout:true}))).status,200);});
}
for(const status of ['refunded','partially_refunded','needs_review']) {
 await test(`${status} order is excluded from reminders`,async()=>{reset();orders[0].payment_status=status;assert.equal((await reminders.processBalanceReminders(request({}))).cancelled,1);assert.equal(emails.length,0);});
}
