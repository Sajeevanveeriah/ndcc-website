...
do $$ declare campaign uuid; first_order uuid; second_order uuid; cancelled_order uuid; numbers integer[]; begin
  select id into strict campaign from public.raffle_campaigns where code='NDCCRRO';
  insert into public.raffle_orders(campaign_id,customer_name,customer_email,quantity,amount_cents,selected_ticket_numbers,payment_reference)
    values(campaign,'Chosen numbers','test@example.com',2,12000,array[201,300],public.allocate_payment_reference('raffle',now())) returning id into first_order;
  ...
  begin
    insert into public.raffle_orders(campaign_id,customer_name,customer_email,quantity,amount_cents,selected_ticket_numbers,payment_reference)
      values(campaign,'Collision','test@example.com',1,6000,array[300],public.allocate_payment_reference('raffle',now()));
  ...
  begin
    insert into public.raffle_orders(campaign_id,customer_name,customer_email,quantity,amount_cents,selected_ticket_numbers,payment_reference)
      values(campaign,'Invalid duplicate','test@example.com',2,12000,array[250,250],public.allocate_payment_reference('raffle',now()));
  ...
  insert into public.raffle_orders(campaign_id,customer_name,customer_email,quantity,amount_cents,payment_reference)
    values(campaign,'Older client','test@example.com',1,6000,public.allocate_payment_reference('raffle',now())) returning id into second_order;
  ...
  insert into public.raffle_orders(campaign_id,customer_name,customer_email,quantity,amount_cents,selected_ticket_numbers,payment_reference)
    values(campaign,'Cancelled checkout','test@example.com',1,6000,array[250],public.allocate_payment_reference('raffle',now())) returning id into cancelled_order;
  ...
  insert into public.raffle_orders(campaign_id,customer_name,customer_email,quantity,amount_cents,selected_ticket_numbers,payment_reference)
    values(campaign,'Reuse released number','test@example.com',1,6000,array[250],public.allocate_payment_reference('raffle',now()));
  begin
