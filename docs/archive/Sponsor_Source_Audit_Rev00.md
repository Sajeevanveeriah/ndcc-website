...
## Sponsor inventory source file

See `data/sponsors/verified-sponsors-20260619.json` for display names, slugs, links, descriptions, logo paths, source URLs, timestamps, and verification status. Entries marked `requires-production-verification` must be researched against official sources before executing the production sponsor upsert.
