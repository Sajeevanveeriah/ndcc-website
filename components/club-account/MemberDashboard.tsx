...
import { CalendarDays, ShoppingBag, HeartHandshake, Newspaper, Utensils, Ticket, UserRound, Settings } from 'lucide-react';
...
type Section = 'overview' | 'purchases' | 'interests' | 'details';
type News = { id: string; title: string; published_at: string | null };
const sections = [
  ['overview', 'Overview', CalendarDays], ['purchases', 'My purchases', ShoppingBag],
  ['interests', 'My interests', HeartHandshake], ['details', 'My details', UserRound],
] as const;
const services = [
...
export default function MemberDashboard({ email, name, status, profileComplete, children }: {
  ...
  useEffect(() => {
    ...
    void Promise.allSettled([
      ...
      load('/api/public/calendar/upcoming?limit=3').then(data => { if (active) setEvents(data); }).catch(() => { if (active) setEventsError(true); }),
  ...
  return <div className="space-y-6">
    ...
    <nav aria-label="My account sections" className="grid grid-cols-2 gap-2 sm:grid-cols-4">{sections.map(([key, label, Icon]) => <button type="button" key={key} aria-pressed={section === key} onClick={() => setSection(key)} className={`flex items-center justify-center gap-2 rounded-xl border px-3 py-3 text-sm font-semibold transition-colors ${section === key ? 'border-maroon-700 bg-maroon-700 text-white' : 'border-edge-subtle bg-surface-card text-content-primary hover:bg-surface-muted'}`}><Icon className="h-4 w-4 shrink-0" aria-hidden="true" />{label}</button>)}</nav>
    <div className="min-w-0 rounded-2xl border border-edge-subtle bg-surface-card p-4 sm:p-7">
      {section === 'details' && <section aria-labelledby="member-details-heading" className="max-w-2xl space-y-5"><div><h2 id="member-details-heading" className="text-2xl font-bold">My details</h2><p className="mt-2 text-content-secondary">Keep your contact details current so the club can reach you when it needs to.</p></div>{children}<div className="flex flex-wrap gap-4 border-t border-edge-subtle pt-5"><Link className="inline-flex items-center gap-2 underline" href="/club-account/reset-password"><Settings className="h-4 w-4" aria-hidden="true" />Change my password</Link><Link className="underline" href="/privacy">How the club uses your details</Link><Link className="underline" href="/contact">Ask about my account</Link></div></section>}
      {section === 'purchases' && <MemberPurchases email={email} />}
      {section === 'interests' && <MemberInterests profileComplete={profileComplete} editProfile={() => setSection('details')} />}
      {section === 'overview' && <div className="space-y-8">
        ...
        {status === 'active' && <Link className="inline-block text-sm underline" href="/raffle/cash">Record trailer raffle cash sales</Link>}
      </div>}
    </div>
  </div>;
}
