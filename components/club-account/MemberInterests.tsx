...
export default function MemberInterests({ profileComplete, editProfile }: { profileComplete: boolean; editProfile: () => void }) {
  ...
  async function persist(next: MemberPreferences, success: string) {
    setBusy(true); setError(''); setMessage('');
    try {
      const result = await clubAccountJsonFetch<{ preferences: MemberPreferences }>('/api/club-account/preferences', { method: 'PUT', body: JSON.stringify(next) });
      setValue(result.preferences); setMessage(success);
    } catch (reason) { setError(reason instanceof Error ? reason.message : 'Your choices could not be saved.'); }
    finally { setBusy(false); }
  }
  async function save(event: React.FormEvent) {
    event.preventDefault();
    await persist(value, 'Your interests and email preference are saved. You can change them here at any time.');
  }
  // One-step unsubscribe: keeps every interest but stops club account emails.
  const unsubscribe = () => persist({ ...value, email_updates: false }, 'You are unsubscribed from club account emails. Your interests are kept.');
