...
// Overview summary of balances owing on the member's most recent website
// orders. Payment itself uses the existing purchase and pay-balance flows.
export default function MemberBalance({ showPurchases }: { showPurchases: () => void }) {
  ...
  useEffect(() => {
    let active = true;
    clubAccountJsonFetch<{ purchases: MemberPurchase[]; total: number }>('/api/club-account/purchases?kind=orders&page=0')
      .then(data => { if (active) { setOwing(data.purchases.filter(order => order.can_pay && order.balance !== null && order.balance > 0)); setMore(data.total > data.purchases.length); } })
  ...
  return <section aria-labelledby="member-balance-heading" className="rounded-xl border border-edge-subtle p-5">
    ...
    {error ? <p className="mt-2 text-sm">Balances are temporarily unavailable. <button type="button" className="underline" onClick={showPurchases}>Open my purchases</button></p>
      ...
      : owing.length === 0 ? <p className="mt-2 text-sm text-content-secondary">Nothing is owing on your recent website orders.{more ? ' Older orders are listed under My purchases.' : ''}</p>
